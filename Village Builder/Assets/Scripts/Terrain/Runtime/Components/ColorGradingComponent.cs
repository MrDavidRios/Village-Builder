namespace UnityEngine.PostProcessing
{
    ...
    public sealed class ColorGradingComponent : PostProcessingComponentRenderTexture<ColorGradingModel>
    {
        ...
        public static Vector3 GetLiftValue(Color lift)
        {
            ...
            var avgLift = (nLift.r + nLift.g + nLift.b) / 3f;

            // Getting some artifacts when going into the negatives using a very low offset (lift.a) with non ACES-tonemapping
            var liftR = (nLift.r - avgLift) * kLiftScale + lift.a;
            var liftG = (nLift.g - avgLift) * kLiftScale + lift.a;
            var liftB = (nLift.b - avgLift) * kLiftScale + lift.a;
        ...
        public static Vector3 GetGammaValue(Color gamma)
        {
            ...
            var avgGamma = (nGamma.r + nGamma.g + nGamma.b) / 3f;

            gamma.a *= gamma.a < 0f ? 0.8f : 5f;
            var gammaR = Mathf.Pow(2f, (nGamma.r - avgGamma) * kGammaScale) + gamma.a;
            var gammaG = Mathf.Pow(2f, (nGamma.g - avgGamma) * kGammaScale) + gamma.a;
            var gammaB = Mathf.Pow(2f, (nGamma.b - avgGamma) * kGammaScale) + gamma.a;

            var invGammaR = 1f / Mathf.Max(kMinGamma, gammaR);
            var invGammaG = 1f / Mathf.Max(kMinGamma, gammaG);
            var invGammaB = 1f / Mathf.Max(kMinGamma, gammaB);
        ...
        public static Vector3 GetGainValue(Color gain)
        {
            ...
            var avgGain = (nGain.r + nGain.g + nGain.b) / 3f;

            gain.a *= gain.a > 0f ? 3f : 1f;
            var gainR = Mathf.Pow(2f, (nGain.r - avgGain) * kGainScale) + gain.a;
            var gainG = Mathf.Pow(2f, (nGain.g - avgGain) * kGainScale) + gain.a;
            var gainB = Mathf.Pow(2f, (nGain.b - avgGain) * kGainScale) + gain.a;

            return ClampVector(new Vector3(gainR, gainG, gainB), 0f, 4f);
        }

        public static void CalculateLiftGammaGain(Color lift, Color gamma, Color gain, out Vector3 outLift,
            out Vector3 outGamma, out Vector3 outGain)
        {
        ...
        public static Vector3 GetSlopeValue(Color slope)
        {
            ...
            var avgSlope = (nSlope.r + nSlope.g + nSlope.b) / 3f;

            slope.a *= 0.5f;
            var slopeR = (nSlope.r - avgSlope) * kSlopeScale + slope.a + 1f;
            var slopeG = (nSlope.g - avgSlope) * kSlopeScale + slope.a + 1f;
            var slopeB = (nSlope.b - avgSlope) * kSlopeScale + slope.a + 1f;
        ...
        public static Vector3 GetPowerValue(Color power)
        {
            ...
            var avgPower = (nPower.r + nPower.g + nPower.b) / 3f;

            power.a *= 0.5f;
            var powerR = (nPower.r - avgPower) * kPowerScale + power.a + 1f;
            var powerG = (nPower.g - avgPower) * kPowerScale + power.a + 1f;
            var powerB = (nPower.b - avgPower) * kPowerScale + power.a + 1f;

            var invPowerR = 1f / Mathf.Max(minPower, powerR);
            var invPowerG = 1f / Mathf.Max(minPower, powerG);
            var invPowerB = 1f / Mathf.Max(minPower, powerB);
        ...
        public static Vector3 GetOffsetValue(Color offset)
        {
            ...
            var avgOffset = (nOffset.r + nOffset.g + nOffset.b) / 3f;

            offset.a *= 0.5f;
            var offsetR = (nOffset.r - avgOffset) * kOffsetScale + offset.a;
            var offsetG = (nOffset.g - avgOffset) * kOffsetScale + offset.a;
            var offsetB = (nOffset.b - avgOffset) * kOffsetScale + offset.a;

            return ClampVector(new Vector3(offsetR, offsetG, offsetB), -0.8f, 0.8f);
        }

        public static void CalculateSlopePowerOffset(Color slope, Color power, Color offset, out Vector3 outSlope,
            out Vector3 outPower, out Vector3 outOffset)
        {
        ...
        private Texture2D GetCurveTexture()
        {
            ...
            for (var i = 0; i < k_CurvePrecision; i++)
            {
                var t = i * k_CurveStep;

                // HSL
                var x = curves.hueVShue.Evaluate(t);
                var y = curves.hueVSsat.Evaluate(t);
                var z = curves.satVSsat.Evaluate(t);
                var w = curves.lumVSsat.Evaluate(t);
                m_pixels[i] = new Color(x, y, z, w);

                // YRGB
                var m = curves.master.Evaluate(t);
                var r = curves.red.Evaluate(t);
                var g = curves.green.Evaluate(t);
                var b = curves.blue.Evaluate(t);
        ...
        private void GenerateLut()
        {
            ...
            if (!IsLogLutValid(model.bakedLut))
            {
                ...
                model.bakedLut = new RenderTexture(k_InternalLogLutSize * k_InternalLogLutSize, k_InternalLogLutSize, 0,
                    GetLutFormat())
                {
            ...
            switch (tonemapping.tonemapper)
            {
                case ColorGradingModel.Tonemapper.Neutral:
                {
                    ...
                    var inBlack = tonemapping.neutralBlackIn * scaleFactor + 1f;
                    var outBlack = tonemapping.neutralBlackOut * scaleFactorHalf + 1f;
                    var inWhite = tonemapping.neutralWhiteIn / scaleFactor;
                    var outWhite = 1f - tonemapping.neutralWhiteOut / scaleFactor;
                    ...
                    lutMaterial.SetVector(Uniforms._NeutralTonemapperParams2,
                        new Vector4(e, f, tonemapping.neutralWhiteLevel,
                            tonemapping.neutralWhiteClip / scaleFactorHalf));
                    break;
                }
            ...
            lutMaterial.SetVector(Uniforms._Balance,
                CalculateColorBalance(settings.basic.temperature, settings.basic.tint));
        ...
        public override void Prepare(Material uberMaterial)
        {
            ...
            var ev = Mathf.Exp(model.settings.basic.postExposure * 0.69314718055994530941723212145818f);
